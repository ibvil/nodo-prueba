...
function listTeamsByCountry() {
  return teams.sort((a, b) => a.country > b.country ? 1 : b.country > a.country ? -1 : 0)
...
function sortTeamsByWins() {
  ...
  teams.forEach(team => {
    let wins = winsByTeams.filter(winsByTeam => winsByTeam.teamId == team.id).map((winsByTeam) => winsByTeam.wins);
    array.push({
      name: team.name,
      wins: wins.reduce((total, winsByTeam) => total + winsByTeam, 0)
    });
  });

  return array.sort((a, b) => a.wins > b.wins ? -1 : b.wins > a.wins ? 1 : 0).map((client) => client.name);
}
...
function leaguesWithWins () {
  ...
  leagues.forEach( league => {
    let leagueTeams = teamsByLeague.filter(team => team.leagueId == league.id).map((team) => team.teamId);
    let wins = winsByTeams.filter(winsByTeam => leagueTeams.includes(winsByTeam.teamId)).map((winsByTeam) => winsByTeam.wins);
...
//Funcion Auxiliar - retorna arreglo de objetos con teamId y wins por liga
function sortTeamsByWinsForLeague (leagueId) {
  let leagueTeams = teamsByLeague.filter(team => team.leagueId == leagueId).map((team) => team.teamId);
  let winsByTeam = winsByTeams.filter(winsByTeam => leagueTeams.includes(winsByTeam.teamId))
...
function leaguesWithTeamWithLestWins () {
  ...
  leagues.forEach(league => {
    ...
    let winsByTeam =  sortTeamsByWinsForLeague(league.id);

    object[league.name] = teams.find( team => team.id == winsByTeam[0].teamId ).name;
...
function leaguesWithTeamWithMostWins () {
  ...
  leagues.forEach(league => {
    ...
    object[league.name] = teams.find( team => team.id == winsByTeam[ winsByTeam.length-1 ].teamId ).name;
...
function sortLeaguesByTeamsByWins () {
  return leaguesWithWins().sort((a, b) => a.wins > b.wins ? -1 : b.wins > a.wins ? 1 : 0).map((league) => league.name);
}
...
function sortLeaguesByTeams () {
  ...
  leagues.forEach(league => {
    ...
    let numberOfTeams = teamsByLeague.filter(team => team.leagueId == league.id).length;
  ...
  return array.sort((a, b) => a.numberOfTeams > b.numberOfTeams ? -1 : b.numberOfTeams > a.numberOfTeams ? 1 : 0).map((league) => league.name);
}
...
function newTeamRanking () {
  ...
  let array = sortTeamsByWins();

  return array.indexOf(teamName) + 1;
}
...
async function getTeamsNamesAsUpperCase() {
  ...
  response = await teamsNamesAsUpperCase();
  // --------------------------------
  console.log('response:');
  console.log(response);
}
// Función que retorna Una promesa Ejercicio 9
function teamsNamesAsUpperCase() {
